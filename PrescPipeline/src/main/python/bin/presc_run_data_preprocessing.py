...
def perform_data_clean(df1,df2):
    try:
        ...
        # 2 Convert city, state, county_name fields to upper case.
    ...
    # 2 Rename the columns
        ...
        df_fact_sel = df2.select(df2.npi.alias("presc_id"),
                                 ...
                                 df2.year_exp.alias("years_of_exp"),
                                 df2.drug_name,
    ...
    # 5 Convert years_of_exp datatype from string to number
        df_fact_sel = df_fact_sel.withColumn('years_of_exp',col("years_of_exp").cast("int"))

    # 6 Combine FirstName And LastName
        df_fact_sel = (df_fact_sel.withColumn("presc_fullname", concat_ws(' ',col('presc_fname'), col("presc_lname"))))
